...
GOOS="$TARGET_OS" GOARCH="$TARGET_ARCH" go build -i -v -ldflags \
    "-X $GO_PROJECT/server.Version=$VERSION -X $GO_PROJECT/server.BuildDate=$BUILD_DATE -X $GO_PROJECT/server.BuildCommit=$GIT_REVISION" \
    -o ${TARGET_NAME} main.go
if [[ $? -ne 0 ]]; then
